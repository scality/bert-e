...
def list_pull_requests(r, key):
    ...
    for i in r.get_pull_requests():
        try:
           p = r.get_build_status(revision=i['source']['commit']['hash'], key=key)
           if p == "NOTSTARTED" and datetime.datetime.strptime(i['updated_on'], '%Y-%m-%dT%H:%M:%S.%f+00:00') > datetime.datetime.now() - datetime.timedelta(time):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Show PRs without build status")
    ...
    parser.add_argument('--owner', action="store", default='scality', help="Repo owner")
    parser.add_argument('--key', action="store", default="pre-merge", help="Pipeline key")
    parser.add_argument('--time', action="store", default="15", help="Go up to x days in the past")
