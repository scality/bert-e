...
class TestWallE(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.bbrepo = BitbucketRepository(client, owner='scality', repo_slug='test_wall_e')
        try:
            cls.bbrepo.delete()
        except:
            pass
    ...
    def create_feature_branch_and_pull_request(
            ...
            file=True):
        self.gitrepo.create_branch(feature_branch, from_branch=from_branch, file=file)
        return self.bbrepo.create_pull_request(
                title='title',
                name='name',
                source={'branch': {'name': feature_branch}},
                destination={'branch': {'name': from_branch}},
                close_source_branch=True,
                reviewers=[{'username': 'scality_wall-e'}],
                description=''
        )['id']
    ...
    def test_bugfix_full_merge_manual(self):
        ...
        with self.assertRaises(AuthorApprovalRequiredException):
            self.wall_e.handle_pull_request('scality', self.bbrepo['repo_slug'], pr_id)
        with self.assertRaises(PeerApprovalRequiredException):
            self.wall_e.handle_pull_request('scality', self.bbrepo['repo_slug'], pr_id, bypass_author_approval=True)
        self.wall_e.handle_pull_request('scality', self.bbrepo['repo_slug'], pr_id, bypass_peer_approval=True,
    ...
    def test_conflict(self):
        ...
        self.wall_e.handle_pull_request('scality', self.bbrepo['repo_slug'], pr_id1, bypass_peer_approval=True,
                                        bypass_author_approval=True)
        with self.assertRaises(ConflictException):
            self.wall_e.handle_pull_request('scality', self.bbrepo['repo_slug'], pr_id2, bypass_peer_approval=True,
                                            bypass_author_approval=True)
        cmd('git merge --abort')
